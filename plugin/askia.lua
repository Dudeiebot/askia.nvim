...
vim.api.nvim_create_user_command("AskToggle", function()
  require("askia").toggle()
end, { desc = "Hide the Claude answer window, or bring the last one back" })

vim.api.nvim_create_user_command("AskClose", function()
  require("askia").quit()
end, { desc = "Close the Claude answer window and discard the answer" })

vim.api.nvim_create_user_command("AskTerminal", function()
  require("askia").escalate()
end, { desc = "Continue this project's Claude session in an interactive terminal" })

vim.api.nvim_create_user_command("AskInfo", function()
  require("askia").info()
end, { desc = "Show this project's Claude session and where it is stored" })

vim.api.nvim_create_user_command("AskReset", function()
  require("askia").reset_session()
end, { desc = "Forget this project's Claude session" })
...
vim.api.nvim_create_user_command("AskClear", function()
  require("askia").clear_references()
end, { desc = "Drop every attached reference" })

vim.api.nvim_create_user_command("AskFollow", function(opts)
  require("askia").follow_up(opts.args)
end, { nargs = "+", desc = "Continue the current Claude conversation" })

vim.api.nvim_create_user_command("AskCancel", function()
  require("askia").cancel()
